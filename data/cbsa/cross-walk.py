...
#   Author: James Graham (NYU, 2018)
#   https://github.com/jagman88/Crosswalk-ZipCode-County-CBSA
#
#   Modified: 05/06/2020 by TomTav
#
# -------------------------------------------------------------------------------
#   INPUTS
# -------------------------------------------------------------------------------
...
skipfooter = (data_year == 2018)*3 + (data_year == 2017)*3
...
if (data_year == 2013):
    usecols = ['CBSA Code', 'Metro Division Code', 'CBSA Title',
               'Metropolitan/Micropolitan Statistical Area']
elif (data_year == 2015):
    usecols = ['CBSA Code', 'Metropolitan Division Code',
...
df_CBSA.loc[(df_CBSA['metromicro'] ==
             'Metropolitan Statistical Area'), 'metromicro'] = 'Metro'
df_CBSA.loc[(df_CBSA['metromicro'] ==
             'Micropolitan Statistical Area'), 'metromicro'] = 'Micro'
...
df_CBSA.loc[(df_CBSA['metrocode'] == 'nan'), 'metrocode'] = df_CBSA.loc[(
    df_CBSA['metrocode'] == 'nan')]['cbsacode']
...
df_zip_fips = pd.read_excel(
    'ZIP_COUNTY_12'+str(data_year)+'.xlsx', dtype={'zip': str, 'county': str})
...
df_zip_fips = df_zip_fips.sort_values(
    'resratio', ascending=False).drop_duplicates('zipcode').sort_index()
...
df_zip_cbsa = pd.read_excel(
    'ZIP_CBSA_12'+str(data_year)+'.xlsx', dtype={'zip': str, 'cbsa': str})
...
# Drop duplicates with lower resratio. i.e. take highest resratio zipcode-CBSA pairs (see note above)
df_zip_cbsa = df_zip_cbsa.sort_values(
    'resratio', ascending=False).drop_duplicates('zipcode').sort_index()
...
df_fips_countyname['CountyFIPS'] = df_fips_countyname['CountyFIPS'].str.zfill(
    3)
df_fips_countyname['FIPS'] = df_fips_countyname['StateFIPS'] + \
    df_fips_countyname['CountyFIPS']
...
df_zip_fips_countyname = pd.merge(
    df_zip_fips, df_fips_countyname, on='FIPS', how='left')

# Merge zip-to-CBSAcode dataframe
df_zip_fips_countyname_cbsa = pd.merge(
    df_zip_fips_countyname, df_zip_cbsa, on='zipcode', how='outer')
...
# Final columns used
usecols = ['zipcode', 'FIPS', 'CountyName',
           'cbsacode', 'cbsatitle', 'metromicro']

usecols = ['zipcode', 'cbsacode']
...
# Sort by CBSA name
df_zip_fips_countyname_cbsaname = df_zip_fips_countyname_cbsaname.sort_values(
    'zipcode')

# Save Cross-walk file to CSV
df_zip_fips_countyname_cbsaname.to_csv(
    'zipcode_cbsa_crosswalk_'+str(data_year)+'.csv', index=False)
